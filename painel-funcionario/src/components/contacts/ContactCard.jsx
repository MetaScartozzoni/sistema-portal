...
const ContactCard = ({ contact, onConvert }) => {
    ...
    >
      <Card className="card-hover h-full flex flex-col">
        ...
        <CardContent className="space-y-3 flex-grow">
          ...
          {contact.contact_reason && <div className="flex items-start space-x-2 text-sm text-gray-300"><MessageSquare className="w-4 h-4 mt-1 flex-shrink-0" /><span>{contact.contact_reason}</span></div>}
        </CardContent>
        <div className="p-6 pt-0">
          <Button className="w-full btn-primary" onClick={() => onConvert(contact.id)}>
