...
const StageChecklistModal = ({ journey, stage, children }) => {
  ...
  const calculateDueDate = () => {
    if (!stage.deadline) return 'N/A';
    
    try {
      const lastCompletedDate = new Date(journey.last_completed_timestamp || patient.first_contact_date || Date.now());
      const surgeryDate = new Date(patient.surgery_date || Date.now());
      ...
      <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400 pt-1">
            Checklist de tarefas para {patient.full_name}
          </DialogDescription>
        </DialogHeader>
