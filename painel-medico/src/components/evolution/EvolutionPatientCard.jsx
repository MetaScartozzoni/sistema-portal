...
import { getPatientEvolutionHistory } from '@/services/api';
import { mapRawEvolution } from '@portal/shared';
...
const EvolutionPatientCard = ({ patient, index, onSaveEvolution }) => {
  ...
  const handleHistoryClick = async () => {
    ...
    try {
  const response = await getPatientEvolutionHistory(patient.id);
  const list = Array.isArray(response.data) ? response.data : [];
  setHistory(list.map(mapRawEvolution));
